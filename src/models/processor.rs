...
impl<'a> Processor<'a> {
    ...
    pub fn process_csv(&self) -> Result<(), AppError> {
        ...
        let result = self.merge_transactions_by_client();
        if result.is_err() {
            print!("rollback");
            return result;
        }
    ...
    fn merge_transactions_by_client(&self) -> Result<(), AppError> {
        ...
        let mut q = ClientQueue::new(ACCOUNT_DIR, dir_paths);
        q.start()?;
        q.stop()?;
        timer.stop();
        Ok(())
